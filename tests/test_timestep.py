...
def test_advance_simulation_steps_exactly_once_per_fixed_dt():
    ...
    leftover = advance_simulation(
        world, accumulator=0.0, frame_time=fixed_dt, intents=[], fixed_dt=fixed_dt
    )
...
def test_advance_simulation_accumulates_partial_frames():
    ...
    acc = advance_simulation(
        world, accumulator=0.0, frame_time=fixed_dt / 2, intents=[], fixed_dt=fixed_dt
    )
    assert len(world.step_calls) == 0
    assert acc > 0.0

    acc = advance_simulation(
        world, accumulator=acc, frame_time=fixed_dt / 2, intents=[], fixed_dt=fixed_dt
    )
    assert len(world.step_calls) == 1
...
def test_advance_simulation_runs_multiple_catchup_steps():
    ...
    leftover = advance_simulation(
        world, accumulator=0.0, frame_time=frame_time, intents=[], fixed_dt=fixed_dt
    )
...
def test_advance_simulation_leftover_always_below_one_fixed_dt():
    ...
    for frame_time in (0.013, 0.027, 0.009, 0.031, 0.002):
        accumulator = advance_simulation(
            world, accumulator, frame_time, intents=[], fixed_dt=fixed_dt
        )
...
def test_clamp_then_advance_bounds_catchup_steps_after_a_stall():
    ...
    advance_simulation(
        world, accumulator=0.0, frame_time=clamped, intents=[], fixed_dt=fixed_dt
    )
